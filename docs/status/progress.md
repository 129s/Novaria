...
- M2 可玩化批次定义为 `issues 14-29`：代码完成并通过自动化后，再进入负责人体验验收。
...
| 14 | P0 | 待返工 | M2 | 输入通道闭环：左键持续 Action、右键瞬时 Interaction、`1-0/滚轮` 槽位切换、按键去抖与单次触发边界；完成标准：事件到意图再到行为链路一致。 |
| 15 | P0 | 待实现 | M2 | 2D 目标解析与可达判定：基于光标定位目标格，统一 `reach=4` 欧氏距离校验，Action/Interaction 共用。 |
...
| 30 | P0 | 待实现 | M2 | 玩家运动模型重构：移除 `W/S/方向键上下` 位移，改为 `A/D` 水平移动 + `Space` 跳跃 + 重力下落，确保“不会悬空静止”。 |
