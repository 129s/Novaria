...
- 已完成 EnTT 玩法链路接入：采集掉落与接触拾取改为 ECS 命令缓冲 + 系统执行，`PlayerController` 不再维护本地掉落容器。
- 已完成 Lua 玩法事件接入：`gameplay.interaction` 与 `gameplay.pickup` 已进入脚本事件总线，并附带分支字段与日志证据。
- 已新增并接入 `novaria_gameplay_issue_e2e_tests`，覆盖输入/ActionBundle/可达判定/配方门槛/掉落拾取/火把光照的黑盒回归。
...
| 18 | P0 | 待确认 | M2 | 已完成背包与热栏数据模型首版：多行热栏行绑定、`Tab` 循环切行、行槽状态可持久，待负责人体验确认。 |
| 19 | P0 | 待确认 | M2 | 已完成 `Esc` 背包/制作入口：打开背包即展示制作区，已移除 Q/F 直连制作路径，待负责人体验确认。 |
| 20 | P0 | 待确认 | M2 | 已完成无形状配方系统：支持 `workbench / wood_sword / torch`，并实现工作台可达前置，待负责人体验确认。 |
| 21 | P0 | 待确认 | M2 | 已完成开局载荷与物品语义首版：初始发放 `镐+斧`，统一工具/方块/武器分类及热栏使用规则，待负责人体验确认。 |
| 22 | P0 | 待确认 | M2 | 已完成煤矿落地：世界生成 `coal_ore`，采集产出 `coal`，资源统计进入 HUD，待负责人体验确认。 |
| 23 | P0 | 待确认 | M2 | 已完成火把闭环：火把配方、放置消耗、不可放水体、火把物品与方块状态同步，待负责人体验确认。 |
| 24 | P0 | 待确认 | M2 | 已完成光照收口：太阳列衰减与火把点光叠加，亮度封顶，昼夜与局部光照共同生效，待负责人体验确认。 |
| 25 | P1 | 待确认 | M2 | 已完成右键交互抽象最小可用：工作台对象交互优先并触发制作入口，保留手持物交互扩展位，待负责人体验确认。 |
| 26 | P1 | 待确认 | M2 | 已完成智能选择 UX 首版：`Ctrl` 切换模式，`Shift` 目标高亮与 `context_slot` 临时切换并自动恢复，待负责人体验确认。 |
| 27 | P1 | 待确认 | M2 | 已完成 HUD 契约首版：生命、热栏行槽、`wood/stone/coal/torch`、智能模式与上下文槽状态可见，待负责人体验确认。 |
| 28 | P1 | 已完成 | M2 | 自动化测试补齐：ActionBundle、可达判定、配方门槛、掉落拾取、火把规则与回归用例。 |
...
| 31 | P0 | 已完成 | M2 | EnTT 玩法接入：ActionBundle 结果写入 ECS 命令缓冲并由系统执行，减少 `PlayerController` 直写世界与资源状态。 |
| 32 | P1 | 已完成 | M2 | Lua 玩法接入：将 Interaction 与掉落副作用暴露为脚本事件/回调，至少支持可观测日志与可配置行为分支。 |
| 33 | P0 | 已完成 | M2 | Issue 级 E2E 测试：新增面向 `issues 14-30` 的黑盒流程测试（输入->动作->资源->配方->光照），用于验收前自动回归。 |
