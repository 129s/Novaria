...
| 2 | 进行中 | 可部署网络传输层：已完成 `UdpTransport` 与 `NetServiceRuntime`（固定 `udp_loopback`，支持本地绑定地址配置、端口/对端配置、最小握手心跳、连接探测指数退避、对端来源校验与动态 peer 采纳），并补充 `novaria_net_smoke` 跨主机联调工具；仍需补跨主机实网压力验证与更完整故障恢复策略。 |
