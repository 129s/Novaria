...
- `ui`：UI 组件与布局系统（纯表现层），只消费稳定 UI Model 并输出 `RenderScene` 覆盖层命令。
...
- 提供 UI 组件与布局系统：把稳定 `UiModel` 转换为 `platform::RenderCommand` 覆盖层列表。
- 维护 UI 视觉系统（颜色/间距/层级）与可复用组件（Panel、Hotbar、Toast、Tooltip、RecipeCard 等）。

**对外保证**

- UI 输出必须是确定性的：同一帧输入与同一份 `UiModel` 必须产生一致的覆盖层命令序列（便于测试与回归）。
- UI 不得回查 `world`/`sim` 的实现：玩法/可达性/配方可用性等判断必须在更高层完成并写入 `UiModel`。

**禁止（关键）**

- 不得持有长期全局状态（除非显式作为 `UiState` 注入并可序列化/可重置）。
- 不得直接读写存档、网络与脚本。
